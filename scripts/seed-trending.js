...
/**
 * Seed script for trending content
 * This script will create sample trending movies and TV shows
 */
...
async function seedTrending() {
  try {
    const filePath = path.join(__dirname, 'trending-seed.json');
    fs.writeFileSync(filePath, JSON.stringify(trendingData, null, 2));
    console.log(`Trending seed data written to ${filePath}`);
    console.log(`Created ${trendingData.length} trending items`);
    
    console.log('\nTo import this data into Strapi:');
    console.log('1. Go to http://localhost:1337/admin');
    console.log('2. Navigate to Content Manager > Trending');
    console.log('3. Import the data from trending-seed.json');
    console.log('\nOr run this script with node to import directly if you have Strapi running.');
    
  } catch (error) {
    console.error('Error creating trending seed data:', error);
  }
}
...
module.exports = { trendingData, seedTrending };
